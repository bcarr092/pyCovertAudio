  ...
  def encode( self, data ):
    if( self.codec and self.checkParamterIsString( data ) ):
      ...
      numberOfBlocks  = \
        int (
          math.ceil (
            float( symbolTracker.getSize() ) / float( self.messageLength )
                    )
            )

      for i in range( numberOfBlocks ):
        blockValues = [ chr( 0 ) for i in range( self.numberOfMessageSymbols ) ]
  ...
  def decode( self, data, errorPositions=None ):
    if( self.codec and self.checkParamterIsString( data ) ):
      ...
      numberOfBlocks  = \
        int (
          math.ceil (
            float( symbolTracker.getSize() ) / float( self.blockLength )
                    )
            )
